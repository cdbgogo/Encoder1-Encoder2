...
# ---------------   DiSAN Interface  ----------------
def disan(rep_tensor, rep_mask, config, scope=None,
    ...
    with tf.variable_scope(scope or 'DiSAN'):
        with tf.variable_scope('ct_attn'):
            ...
            # print('fw_res is {}'.format(fw_res))
            # print('bw_res is {}'.format(bw_res))
            seq_rep = tf.concat([fw_res, bw_res], -1)
            seq_rep = tf.reshape(seq_rep, [-1, config.max_sequence_length, 2*config.emb_size])  # fixme debug by hr
            # print('seq_rep is {}'.format(seq_rep))
...
def directional_attention_with_dense(rep_tensor, rep_mask, direction=None, scope=None,
    ...
    with tf.variable_scope(scope or 'directional_attention_%s' % direction or 'diag'):
        ...
        rep_mask = tf.reshape(rep_mask, (bs, -1))  # fixme hr, fix debug when b_s is 1
...
def multi_dimensional_attention(rep_tensor, rep_mask, scope=None,
    ...
    with tf.variable_scope(scope or 'multi_dimensional_attention'):
        ...
        map2_masked = tf.reshape(map2_masked, [bs, sl, vec])  # fixme debug by hr
...
def linear(args, output_size, bias, bias_start=0.0, scope=None, squeeze=False, wd=0.0, input_keep_prob=1.0,
    ...
    # flat_args = [flatten_new(arg) for arg in args]  # for dense layer [(-1, d)]
